...
def optimized_Sx2_pulses_by_partial_repetition(
    num_attempts:int=50, 
    num_runs_per_attempt:int=int(8*1e2), 
    num_moments:int=40, 
    num_free_params:int=25
) -> LearnedResults:
    ...
    for attempt_ind in range(num_attempts):
        ...
        param_config, operations = _sx_sequence_params(standard_operations, sigma=0.00, theta=theta, num_free_params=num_free_params)            
        print(strings.num_out_of_num(attempt_ind+1, num_attempts))
